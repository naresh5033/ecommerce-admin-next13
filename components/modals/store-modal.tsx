...
const formSchema = z.object({
  name: z.string().min(1), //min of 1 char require to create our store
});
...
export const StoreModal = () => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      setLoading(true);
      const response = await axios.post('/api/stores', values); //the post mehod that i created in the api/store
